...
def cal_similarity(newCase, cases, similaritys):
    similarities=[]
    similarities.append(calDiscrete(newCase.get_property(), cases))


def generateSimilarity(newCases):
    excelPath = "src\\cases.xlsx"
    ...
    for j in range(0, len(newCases)):
        ...
        for m in range(0, len(cases[j])):
            ...
            if m == propertyIndex:
                temp = calDiscrete(newCase.get_property(), cases[:, m])
            elif m == edIndex:
                temp = calRelief(newCase.get_elevationD(), cases[:, m])
            elif m == areaIndex:
                temp = calArea(newCase.get_area(), cases[:, m])
            elif m == msIndex:
                temp = calSlope(newCase.get_meanS(), cases[:, m])
            elif m == upIndex:
                temp = calDepth(newCase.get_up(), cases[:, m])
            elif m == downIndex:
                temp = calDepth(newCase.get_down(), cases[:, m])
            elif m == reIndex:
                temp = calResolution(newCase.get_resolution(), cases[:, m])
            elif m == sdhIndex:
                temp = calResolution(newCase.get_SDH(), cases[:, m])
            similaritys[:, m] = temp
    ...
    envsTable = pd.read_excel("src\\envClass.xlsx", sheet_name="class")
    ...
    for i in range(0, len(newCases)):
        ...
        print('')
    wb.save('src\\finalStatistic_MS.xls')
...
if __name__ == '__main__':
    ...
    down = 10
    property = 15
    DEMfile = "src\\dem_xc.tif"
    #initialization type1
    case1 = NewCase()
    case1.set_property(15)
    case1.set_up(10)
    case1.set_down(30)
    case1.set_resolution(30)

    # initialization type2
    case2 = NewCase.from_DEMfile(up, down, property, DEMfile)

    newCases = []
    newCases.append(case1)
    newCases.append(case2)
    generateSimilarity(newCases)
    '''
    # 调用get_方法获取参数值
    print(case.get_up())  
    print(case.get_down()) 
    print(case.get_property())
    print(case.get_resolution())
    print(case.get_area())
    print(case.get_SDH())
    print(case.get_meanS())
    print(case.get_elevationD())
    '''
